...
SPECIES: list[dict] = [
{
    ...
    },
    {
        ...
        "classifier_description": (
            ...
            "fifth spines. NOTE ON SCULPIN CONFUSION: this checkerboard blotch pattern can superficially "
            "resemble Sculpin's mottled camouflage at a glance, since both are 'irregular' rather than "
            "uniform round dots — but blotch shape alone should NOT decide between them. Calico Bass has "
            "smooth, unarmored skin and a small sleek head with no bony ridges, no spines, and no fleshy "
            "skin tabs anywhere on the head or jaw. If the head is smooth and unarmored, it is NOT a "
            "Sculpin, regardless of how blotchy, checkered, or irregular the body pattern looks. "
            "COMMONLY CONFUSED WITH: Barred Sand Bass and Spotted Bay Bass. DIFFERENTIATION: Calico Bass "
            "has square blotches instead of vertical bars, and its top fin is relatively flat, lacking "
            "the massively elongated third-spine spike found on both Sand Bass and Spotted Bay Bass."
        )
    },
    {
    ...
    },
    {
        ...
        "classifier_description": (
            ...
            "regular checkerboard. Use body pattern only to CONFIRM a call already supported by head "
            "armor — do not use blotchy or irregular-looking pattern by itself to call Sculpin, since "
            "Calico Bass's square checkerboard blotches can also look 'irregular' at a glance despite "
            "belonging to a completely different, smooth-headed species. IMPORTANT — DO NOT use a "
            ...
            "non-uniform body pattern. Never call Sculpin based on body pattern, dorsal fin spikiness, "
