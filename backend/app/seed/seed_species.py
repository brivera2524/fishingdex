...
SPECIES: list[dict] = [
{
    ...
    },
    {
        ...
        "classifier_description": (
            ...
            "all. Instead they show a paler cream, tan, or pale-yellow background marked with distinct "
            "dark brown 'saddle' blotches or loose diagonal/vertical banding down the back — a pattern "
            "that can look mottled or irregular at a glance and should NOT be dismissed as 'doesn't "
            "match Calico Bass, must be something else.' This juvenile phase is still Calico Bass. Other "
            "reliable features that hold regardless of color phase or fish size: a dark horizontal bar "
            "or stripe running through the eye; bright yellow-to-orange pelvic and anal (lower) fins, "
            "distinctly more saturated and colorful than Sculpin's fins, which stay dull and within the "
            "body's own red/brown/orange camouflage tones; and completely smooth, unarmored skin with a "
            "small sleek head — no bony ridges, no spines, and no fleshy skin tabs anywhere on the head "
            "or jaw. ABSENCE OF HEAD ARMOR RULES OUT SCULPIN BY ITSELF: if the head is smooth with no "
            "bony ridges or fleshy tabs, this is NOT a Sculpin — full stop — no matter how blotchy, "
            "saddle-patterned, checkered, or irregular the body coloring looks, and regardless of fish "
            "size. KEY DIAGNOSTIC AGAINST SCULPIN — TRUE WHITE COLORING (adult/larger fish): on fish "
            "showing the adult checkerboard, the blotches include genuinely WHITE or pale patches "
            "contrasting against the darker olive-brown blotches — real white, not just a lighter shade "
            "of tan/brown. Sculpin does NOT have true white patches — its camouflage stays entirely "
            "within a warm red/orange/brown range. If you see distinct white or pale-white blotches "
            "anywhere on the body, it is Calico Bass, not Sculpin, regardless of how irregular or "
            "non-checkerboard the pattern looks. Calico Bass also has smaller, pointed pectoral fins "
            "rather than Sculpin's large round pleated fan-like pectoral fins. COMMONLY CONFUSED WITH: "
            "Barred Sand Bass, Spotted Bay Bass, and Sculpin. DIFFERENTIATION FROM SAND/SPOTTED BASS: "
            ...
            "Bay Bass. DIFFERENTIATION FROM SCULPIN: judge by head armor (present=Sculpin, "
            "absent=Calico) and fin color (dull=Sculpin, bright yellow-orange=Calico) — never by how "
            "blotchy or non-uniform the body pattern looks, since juvenile Calico Bass patterning can "
            "look just as irregular as Sculpin's camouflage."
        )
    },
    {
    ...
    },
    {
        ...
        "classifier_description": (
            "Chunky, heavy-bodied fish with ROUGH, textured skin. HARD GATE — HEAD ARMOR IS REQUIRED AND "
            "ITS ABSENCE ALONE DISQUALIFIES SCULPIN, CHECK THIS FIRST: a fish can only be classified as "
            "Sculpin if the head is large, blocky, and heavily armored — covered in sharp bony ridges "
            "and small fleshy skin tabs/flaps (like tiny flaps of skin sticking out, especially around "
            "the eyes and mouth). If you look at the head and do NOT clearly see bony ridges or fleshy "
            "tabs, that absence is sufficient BY ITSELF to rule out Sculpin — do not let a blotchy body, "
            "a saddle-patterned back, a checkered pattern, an irregular pattern, a spiky-looking dorsal "
            "fin, or reddish/brownish/tan coloring override a smooth head. This especially applies to "
            "SMALL OR JUVENILE FISH: young Paralabrax bass (Spotted Bay Bass, Barred Sand Bass, and "
            "especially Calico Bass) often show a mottled, saddle-blotched, or irregular juvenile "
            "coloring on a pale cream/tan background that superficially resembles Sculpin's camouflage "
            "— but these juvenile bass still have smooth, unarmored heads, so head smoothness overrides "
            "any resemblance in body pattern, every time. ANOTHER DISQUALIFIER — FIN COLOR: Sculpin's "
            "pelvic and anal (lower) fins stay dull and within the body's own red/brown/orange "
            "camouflage tones. If the lower fins are bright, saturated yellow-to-orange (distinctly more "
            "vivid than the body color), that points away from Sculpin and toward a bass species "
            "instead — Sculpin does not have vividly colored fins. SECOND STRONG CUE — PECTORAL FIN "
            "SHAPE: Sculpin's pectoral (front/side) fins are distinctively large, rounded, and "
            "fan-shaped, with a pleated or ribbed look to the individual fin rays, like an open hand fan "
            "— noticeably different from the smaller, more angular, pointed pectoral fins on Paralabrax "
            "bass (Spotted Bay Bass, Barred Sand Bass, Calico Bass). A big, round, pleated fan-like "
            "pectoral fin supports Sculpin only when paired with confirmed head armor; a smaller, "
            "pointed pectoral fin argues against Sculpin regardless of body pattern. THIRD CUE, "
            ...
            "NOT have genuinely white or pale-white patches. If the fish shows true white or pale cream "
            "blotches contrasting against darker blotches (even if the pattern shape itself looks "
            "irregular or saddle-like, as on a smaller or juvenile fish), that points to Calico Bass, "
            "not Sculpin — rule out Sculpin immediately in that case. Use body pattern only to CONFIRM a "
            "call already supported by head armor — never use blotchy, irregular, or saddle-patterned "
            "body coloring by itself to call Sculpin, since juvenile Calico Bass in particular can look "
            "just as irregular at a glance despite belonging to a completely different, smooth-headed "
            "species. IMPORTANT — DO NOT use a raised/spiky dorsal fin alone as Sculpin evidence: "
            "Sculpin's dorsal spines are genuinely thick and heavy, but Paralabrax bass all also have a "
            "spiky erect anterior dorsal fin when handled — a spiky-looking fin on its own does NOT mean "
            "Sculpin. Likewise, DO NOT weight overall body color/tone (red/orange/brown/tan/cream vs. "
            "grey/olive) as decisive on its own — a brownish, tan, or cream-colored bass is still a "
            "bass; color and pattern vary enormously with age, habitat, and lighting. DECISION RULE: "
            "only classify as Sculpin if the HEAD shows bony ridges or fleshy tabs (strictly required) "
            "— optionally strengthened by a large, round, pleated fan-like pectoral fin, dull "
            "non-saturated fin color, and/or a blotchy, non-uniform, NON-WHITE body pattern. Never call "
            "Sculpin based on body pattern, dorsal fin spikiness, or color alone if the head is smooth — "
            "this is the single most common error to avoid, especially on small or juvenile fish. NOTE "
            "ON SPECKLEFIN MIDSHIPMAN OVERLAP: Specklefin "
