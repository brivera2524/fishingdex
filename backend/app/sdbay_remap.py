"""Shared lon/lat regridding + compact storage helpers for the San Diego Bay
current simulation, used by both the (offline, GPU-driven) bank generation
script and the (lightweight, request-time) bank lookup in app/ocean_sim.py.

The solver's native grid is regular in UTM meters, not lon/lat degrees, so
serving it to leaflet-velocity (which wants a regular lon/lat grid) needs a
one-time nearest-neighbor resample onto the output grid defined here.
"""
...
# Covers San Diego Bay's interior only; the HFRadar layer (app/ocean_current.py)
# already covers the open coastal water outside it. Bounds match the solver
# grid's real wet-cell extent (lon -117.240..-117.100, lat 32.603..32.741)
# plus a small margin.
OUT_LAT_MIN, OUT_LAT_MAX = 32.60, 32.745
OUT_LON_MIN, OUT_LON_MAX = -117.245, -117.095
OUT_NY, OUT_NX = 230, 230

# int8 quantization for storing u/v compactly in the bank (same idea as the
# reference chartplotter's binary payload). Real speeds here are well under
# 1 m/s (validated peak ~0.68 m/s, see docs/LIMITATIONS.md); clipping to
# +-1 m/s trades a little precision on rare fast jets for 4x smaller storage
# than float32.
